...
def create_widgets(root, genres):
    ...
    # Movies Treeview with Scrollbars
...
def recommend(genre_vars, tree, genres):
    ...
    if not selected_genres:
        animate_messagebox("No Selection", "Please select at least one genre!")
        return
    ...
    filtered_movies = [
        ...
        for movie in data
        if all(genre in [g for g in genres if movie[g] == '0'] for genre in selected_genres)
    ]
